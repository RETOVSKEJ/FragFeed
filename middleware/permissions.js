...
function authUser(req,res,next)
{
    ...
    res.status(401); throw new Error('You are not authorized')
}
...
function authRoles(role){
    return (req, res, next) => {
        ...
        res.status(403); throw new Error(403);
    } 
}
...
async function canAddPost(req,res,next){
    ...
    res.status(403); throw new Error("You are not allowed to add posts")
}
...
async function canEditPost(req,res,next){   
    ...
    res.status(403); throw new Error("You are not allowed to edit this post");
}

async function canDeletePost(req, res, next){
    if(req.session.post.id != req.params.id)       // if the post is not in the session, get it from the db
        req.session.post = await Post.findOne({id: req.params.id}).populate('author', '-password').exec()

    if (req.user.role === ROLES.ADMIN || req.session.post?.author?._id === req.user._id.toString())
    return next();

    res.status(403); throw new Error("You are not allowed to delete this post");
}
...
async function canCommentPost(req,res,next){
    ...
    res.status(403); throw new Error("You are not allowed to comment this post")
}
