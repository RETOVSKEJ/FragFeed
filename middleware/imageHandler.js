...
const uploadCompressedDisk = multer({storage: new MyCustomStorage({
    destination: function(req, file, cb){
        cb(null, pathModule.resolve('public', 'assets', 'uploads', 'uploaded'))
    },
    //// MULTER BUG   -> cannot set filename here, destination is full path to the new file
}), limits: {fileSize: 5_100_000}})     // jesli przekroczymy, wyskoczy sharp Error: VipsJpeg:
...
async function storeImage(data, uploadedFilename, newFilename){
    ...
    let filePath = pathModule.resolve('public', 'assets', 'uploads', `${newFilename}${fileExt}`)
